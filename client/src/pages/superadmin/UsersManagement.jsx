...
import ManageUserModal from '../../components/forms/ManageUserModal';

export default function UsersManagement() {
  const { users, stats, fetchUsers, isLoading: usersLoading, deleteUser, updateUserStatus, resetUserPassword } = useUserStore();
  ...
  const getCompanyName = (user) => {
    ...
    if (user.companyId) {
      const co = companies.find(c => c._id === user.companyId || c.id === user.companyId);
      return co ? co.name : 'Unknown';
    }
    return 'No Company Attached';
  };
  ...
  const filteredUsers = users.filter(user => {
    ...
    const coId = user.company?._id || user.companyId;
    const matchCompany = filterCompany === 'all' || coId === filterCompany;
  ...
  const handleStatusToggle = async (user) => {
    ...
    if (reason !== null) {
      await updateUserStatus(user._id || user.id, newStatus, reason);
    }
  };
  ...
  const handleResetPassword = async (user) => {
    ...
    if (np && np.length >= 6) {
      const res = await resetUserPassword(user._id || user.id, np);
    ...
    <div className="w-full pt-2 font-sans relative">
      <div className="mb-10 flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <Users className="text-blue-500" size={32} /> User Directory
          </h1>
          <p className="text-slate-400 mt-2 font-medium">Control identities, companies, and roles globally.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          ...
          <div><div className="text-3xl font-black text-white">{stats.totalUsers || users.length}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Total Base</div></div>
        </motion.div>
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          <div className="p-3 bg-emerald-500/20 text-emerald-400 rounded-xl"><ShieldCheck size={24} /></div>
          <div><div className="text-3xl font-black text-white">{stats.activeUsers || users.filter(u=>u.status==='active').length}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Active nodes</div></div>
        </motion.div>
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          <div className="p-3 bg-red-500/20 text-red-400 rounded-xl"><ShieldAlert size={24} /></div>
          <div><div className="text-3xl font-black text-white">{stats.suspendedUsers || users.filter(u=>u.status==='suspended').length}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Suspended</div></div>
        </motion.div>
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          <div className="p-3 bg-purple-500/20 text-purple-400 rounded-xl"><Briefcase size={24} /></div>
          <div><div className="text-3xl font-black text-white">{stats.withCompany || Object.values(users).filter(v => v.companyId || v.company).length}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">With Company</div></div>
        </motion.div>
      </div>
      ...
      <div className="bg-[#111620]/80 backdrop-blur-2xl border border-white/5 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden min-h-[600px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 relative z-10">
          ...
          <select value={filterCompany} onChange={(e) => setFilterCompany(e.target.value)} className="w-full bg-[#1d2633] border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 focus:ring-blue-500/50 shadow-inner text-sm appearance-none">
          ...
          <table className="w-full text-left">
            <thead className="bg-[#181f2b]/80 border-b border-white/5">
              <tr>
                ...
                <th className="p-5 text-[11px] font-bold text-slate-500 uppercase tracking-widest border-b border-white/5">Company</th>
