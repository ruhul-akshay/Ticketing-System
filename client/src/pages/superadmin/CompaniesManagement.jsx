...
export default function CompaniesManagement() {
  const { companies, stats, fetchCompanies, fetchStats, refreshAnalytics, isRefreshing, isLoading, addCompany, updateCompany, deleteCompany, suspendCompany } = useCompanyStore();
  ...
  const loadData = useCallback(() => {
    fetchCompanies({ search: searchTerm, status: statusFilter !== 'all' ? statusFilter : undefined, erpName: erpFilter !== 'all' ? erpFilter : undefined });
    fetchStats();
  }, [searchTerm, statusFilter, erpFilter, fetchCompanies, fetchStats]);
  ...
  const submitForm = async (e) => {
    ...
    let res;
    if (modalMode === 'create') res = await addCompany(payload);
    else res = await updateCompany(selectedCompany._id || selectedCompany.id, payload);
  ...
  const submitDelete = async () => {
    await deleteCompany(selectedCompany._id || selectedCompany.id);
    setModalMode(null);
  };
    ...
    <div className="w-full relative font-sans min-h-screen pt-4">
      <div className="mb-8 flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <Building2 className="text-[#ED1B2F]" size={32} /> Corporate Entities
          </h1>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={openCreateModal} className="px-6 py-2.5 bg-gradient-to-r from-red-600 to-[#ED1B2F] hover:from-red-500 hover:to-red-400 rounded-xl text-white text-[13px] font-bold flex items-center gap-2 transition-all shadow-[0_0_20px_rgba(237,27,47,0.3)]">
            <Plus size={16} strokeWidth={3} /> Create Company
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          ...
          <div><div className="text-3xl font-black text-white">{stats?.totalCompanies || companies.length}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Total Entities</div></div>
        </motion.div>
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          <div className="p-3 bg-emerald-500/20 text-emerald-400 rounded-xl"><Users size={24} /></div>
          <div><div className="text-3xl font-black text-white">{stats?.totalUsers || companies.reduce((a,b)=>a+(b.totalUsers||b.employeeCount||0),0)}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Total Users</div></div>
        </motion.div>
        <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          <div className="p-3 bg-blue-500/20 text-blue-400 rounded-xl"><Ticket size={24} /></div>
          <div><div className="text-3xl font-black text-white">{stats?.totalTickets || companies.reduce((a,b)=>a+(b.totalTickets||0),0)}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Total Tickets</div></div>
        </motion.div>
         <motion.div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-2xl flex items-center gap-4 shadow-xl">
          <div className="p-3 bg-purple-500/20 text-purple-400 rounded-xl"><Database size={24} /></div>
          <div><div className="text-3xl font-black text-white">{companies.filter(c => c.erpDetails?.erpName === 'SAP B1').length}</div><div className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">SAP B1 Nodes</div></div>
        </motion.div>
      </div>
      ...
      <div className="bg-[#111620]/80 backdrop-blur-2xl border border-white/5 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden min-h-[600px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 relative z-10">
          <div className="md:col-span-2">
            <input type="text" placeholder="Search entity string..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#1d2633] border border-white/5 rounded-xl px-4 py-3 text-white placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-red-500/50 shadow-inner text-sm" />
          </div>
        ...
        <div className="overflow-x-auto relative z-10">
          <table className="w-full text-left">
             <thead className="bg-[#181f2b]/80 border-b border-white/5">
              <tr>
                <th className="p-5 text-[11px] font-bold text-slate-500 uppercase tracking-widest rounded-tl-xl border-b border-white/5">Company</th>
            ...
            <tbody className="text-white">
               {isLoading && companies.length === 0 ? (
                 ...
                 <React.Fragment key={c.id || c._id}>
                   <motion.tr initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }} className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors group ${c.isNewCompany ? 'bg-blue-500/5' : ''}`}>
                      <td className="p-5">
                       <div className="flex items-center gap-5">
                        ...
                        <div>
                          <div className="font-black text-[16px] flex items-center gap-2 text-white">
                            {c.name}
                            {c.isNewCompany && <Badge color="blue" size="sm">NEW</Badge>}
                      ...
                      <td className="p-5">
                          <div className="flex flex-col items-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                             ...
                             <button onClick={() => { setSelectedCompany(c); setModalMode('delete'); }} className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-slate-400 hover:text-red-500 bg-white/5 py-1.5 px-3 rounded-lg border border-transparent hover:border-red-500/30 transition-all cursor-pointer">
                                 ...
                                 <div className="md:col-span-4 bg-white/5 border border-white/10 rounded-3xl p-6 mt-2 relative overflow-hidden group/amc">
                                   ...
                                   <div className="flex justify-between items-start relative z-10">
                                     ...
                                     <div className="flex gap-4 items-center">
                                       ...
                                       <Button variant="ghost" size="sm" className="bg-blue-600/10 text-blue-400 border border-blue-500/20 hover:bg-blue-600/20 h-10 px-4 rounded-xl font-bold" onClick={() => { setSelectedCompany(c); setModalMode('renew'); }}>
                                         Renew Contract
                                       </Button>
                                     </div>
                                   </div>
      ...
      <Modal isOpen={modalMode === 'create' || modalMode === 'edit'} onClose={() => setModalMode(null)} title={modalMode === 'create' ? "Create New Company" : "Edit Company"} size="xl">
        <form onSubmit={submitForm} className="space-y-6">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <Input label="Company Name *" value={formData.name} onChange={e => setFormData(p=>({...p, name: e.target.value}))} required />
              ...
              <div className="space-y-4">
                 <div>
                   ...
                   <select value={formData.erpDetails.erpName} onChange={e=>setFormData(p=>({...p, erpDetails: {...p.erpDetails, erpName: e.target.value}}))} className="w-full bg-[#111620] border border-white/5 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-500/50 appearance-none">
                     <option value="">No ERP Specificed</option>
                     {erpOptions.map(e=><option key={e} value={e}>{e}</option>)}
                   </select>
                 </div>
                 ...
                 {formData.erpDetails.erpName === 'SAP B1' && (
                   <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-4">
                      ...
                      {formData.erpDetails.sapSupportAMC.status === 'Active' && (
                         <>
                            <div className="grid grid-cols-2 gap-4 p-4 border border-emerald-500/20 bg-emerald-500/5 rounded-xl">
           ...
           <div className="flex justify-end gap-3 mt-8">
            ...
            <Button variant="primary" type="submit" disabled={!formData.name} icon={Send}>{modalMode === 'create' ? 'Create Company' : 'Save Changes'}</Button>
          </div>
        </form>
      </Modal>

      <Modal isOpen={modalMode === 'delete'} onClose={() => setModalMode(null)} title="Delete Company" size="md">
         <div className="text-center py-6">
            ...
            <h3 className="text-2xl font-black text-white mb-2">Delete {selectedCompany?.name}?</h3>
            <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">Are you sure you want to permanently delete this company? This action cannot be undone.</p>
      ...
      <Modal isOpen={modalMode === 'renew'} onClose={() => setModalMode(null)} title="Renew Support Contract" size="md">
         <form onSubmit={async (e) => {
            ...
            const { renewCompanyContract } = useCompanyStore.getState();
            await renewCompanyContract(selectedCompany._id || selectedCompany.id, payload);
            setModalMode(null);
         }} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Input name="fromDate" label="New From Date" type="date" defaultValue={selectedCompany?.erpDetails?.sapSupportAMC?.toDate ? new Date(selectedCompany.erpDetails.sapSupportAMC.toDate).toISOString().split('T')[0] : ''} required />
              <Input name="toDate" label="New To Date" type="date" required />
            </div>
            <div className="grid grid-cols-2 gap-4 mt-2">
               <div>
                 <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Support Type</label>
                 <select name="sapSupportAMCType" defaultValue={selectedCompany?.erpDetails?.sapSupportAMCType || 'Unlimited'} className="w-full bg-[#111620] border border-white/5 text-white rounded-lg px-3 py-2 text-sm">
                   {supportAMCTypeOptions.map(o=><option key={o} value={o}>{o}</option>)}
                 </select>
               </div>
               <Input name="sapSupportHourlyCap" label="New Hourly Cap" type="number" min="0" defaultValue={selectedCompany?.erpDetails?.sapSupportHourlyCap || 0} />
            </div>
