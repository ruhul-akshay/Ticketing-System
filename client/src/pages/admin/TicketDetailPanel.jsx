            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-1.5">
                ...
                <span className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-widest border ${
                  status === 'Resolved' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'
                }`}>
                  {status}
                </span>
              </div>
              ...
              <div className="space-y-8 bg-black/40 rounded-[3rem] p-10 border border-white/5 relative overflow-hidden">
                ...
                {(!ticket.original?.remarks || ticket.original.remarks.length === 0) ? (
                  ...
                  ticket.original.remarks.map((remark, index) => {
                    ...
                    return (
                      <div key={index} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'} w-full group animate-in fade-in slide-in-from-bottom-6 duration-700`}>
                        <div className={`max-w-[85%] flex flex-col ${isAdmin ? 'items-end' : 'items-start'}`}>
                          <div className={`flex items-center gap-3 mb-3 px-4 ${isAdmin ? 'flex-row-reverse' : 'flex-row'}`}>
                            <div className={`w-9 h-9 rounded-2xl flex items-center justify-center text-[12px] font-black shadow-2xl transition-all group-hover:rotate-6 ${
                              isAdmin ? 'bg-blue-600 text-white ring-4 ring-blue-600/10' : 'bg-slate-700 text-slate-300 ring-4 ring-slate-700/10'
                            }`}>
                              {senderName[0].toUpperCase()}
                            </div>
                            <div className={`flex flex-col ${isAdmin ? 'items-end' : 'items-start'}`}>
                              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{isMe ? 'Internal' : isAdmin ? 'Support Node' : 'Client Node'}</span>
                          ...
                          <div className={`relative p-6 rounded-[2.5rem] shadow-2xl border transition-all duration-500 hover:scale-[1.01] ${
                            isAdmin ? 'bg-blue-600 border-blue-400/30 text-white rounded-tr-none' : 'bg-[#1e293b] border-white/5 text-slate-200 rounded-tl-none'
                          }`}>
                            <p className="text-[15px] leading-relaxed whitespace-pre-wrap font-medium">{remark.text}</p>
                            
                            {remark.attachments?.length > 0 && (
                              <div className={`mt-6 pt-5 border-t space-y-3 ${isAdmin ? 'border-white/20' : 'border-white/5'}`}>
                                {remark.attachments.map((file, fIdx) => (
                                  <button key={fIdx} onClick={(e) => handleDownloadAttachment(e, rawTicketId, file._id, file.originalName || file.filename)} className={`flex items-center gap-4 p-4 rounded-2xl border transition-all w-full text-left group/file ${
                                    isAdmin ? 'bg-white/10 border-white/10 hover:bg-white/20' : 'bg-black/20 border-white/5 hover:bg-black/30'
                                  }`}>
                                    <div className={`p-2.5 rounded-xl ${isAdmin ? 'bg-white/20' : 'bg-blue-600/20 text-blue-400'}`}>
