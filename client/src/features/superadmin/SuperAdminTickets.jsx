...
import { Ticket, Search, Filter, Plus, RefreshCw, AlertCircle, Clock, CheckCircle, ChevronLeft, ChevronRight, Paperclip, XCircle, Download, Building2 } from 'lucide-react';
...
export default function SuperAdminTickets() {
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(t => {
      ...
      return matchesSearch && matchesClient && matchesStatus && matchesPriority && matchesConsultant && matchesDate;
    });
      ...
      <div className="bg-[#111620]/80 backdrop-blur-2xl border border-white/5 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-8 shadow-2xl relative overflow-hidden min-h-[600px] flex flex-col">
        ...
        <div className="bg-[#181f2b]/80 border border-white/5 rounded-2xl p-4 mb-6 relative z-10 flex flex-col gap-4">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              <select value={sortBy} onChange={e => {setSortBy(e.target.value); setCurrentPage(1);}} className="bg-[#111620] border border-white/5 rounded-xl px-4 py-3 text-white text-[13px] font-bold focus:outline-none focus:border-blue-500/50 shadow-inner">
                    ...
                    <td className="p-4 align-middle">
                       <div className="flex flex-col gap-1.5">
                         ...
                         <span className={`font-extrabold text-[14px] flex items-center gap-1.5 truncate transition-colors ${
                           ...
                           {t.title}
                           {t.attachments?.length > 0 && <Paperclip size={13} className="text-blue-400 shrink-0" />}
                         </span>
       ...
       {forwardingTicket && (
         <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
           <div className="absolute inset-0 bg-[#020617]/80 backdrop-blur-sm" onClick={() => {
             setForwardingTicket(null);
             setModalCcConsultantIds([]);
             setModalCcEmails('');
             setCcDropdownOpen(false);
           }} />
           <div className="bg-white dark:bg-[#111620] border border-slate-200 dark:border-white/10 w-full max-w-md rounded-2xl p-6 shadow-2xl relative z-10 space-y-4">
             ...
             <div className="space-y-4">
               <div className="flex flex-col gap-1.5">
                 <label className="text-[11px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-widest">Select Consultant to Forward To</label>
                 <select
                   value={modalConsultantId}
                   onChange={(e) => setModalConsultantId(e.target.value)}
                   className="bg-slate-50 dark:bg-[#181f2b] border border-slate-200 dark:border-white/5 rounded-xl px-3.5 py-2.5 text-slate-800 dark:text-white text-[12px] font-bold focus:outline-none focus:border-blue-500/50 cursor-pointer w-full shadow-inner"
                 >
               ...
               <div className="flex flex-col gap-1.5 relative">
                 <label className="text-[11px] font-bold text-slate-455 dark:text-slate-400 uppercase tracking-widest">CC Consultants (Optional)</label>
                 <div 
               ...
               <div className="flex flex-col gap-1.5">
                 <label className="text-[11px] font-bold text-slate-455 dark:text-slate-400 uppercase tracking-widest">Additional CC Email Addresses (Optional)</label>
                 <input
                   type="text"
                   placeholder="e.g. manager@example.com, developer@example.com"
                   value={modalCcEmails}
                   onChange={(e) => setModalCcEmails(e.target.value)}
                   className="bg-slate-50 dark:bg-[#181f2b] border border-slate-200 dark:border-white/5 rounded-xl px-3.5 py-2.5 text-slate-800 dark:text-white text-[12px] font-bold focus:outline-none focus:border-blue-500/50 w-full placeholder:text-slate-400 shadow-inner"
                 />
                 <span className="text-[9px] text-slate-500 font-medium">Separate multiple email addresses with commas.</span>
               </div>

               <div className="flex flex-col gap-1.5">
                 <label className="text-[11px] font-bold text-slate-455 dark:text-slate-400 uppercase tracking-widest">Remarks</label>
                 <textarea
                   placeholder="Reason for forwarding..."
                   value={modalRemarks}
                   onChange={(e) => setModalRemarks(e.target.value)}
                   className="bg-slate-50 dark:bg-[#181f2b] border border-slate-200 dark:border-white/5 rounded-xl p-3 text-[12px] text-slate-800 dark:text-white focus:outline-none focus:border-blue-500/50 min-h-[80px] resize-none w-full shadow-inner placeholder:text-slate-400"
                 />
               </div>
               <div className="flex justify-end gap-3 pt-2">
                 <button onClick={() => {
                   ...
                   setModalCcEmails('');
                   setCcDropdownOpen(false);
                 }} className="px-4 py-2 text-[12px] font-bold text-slate-500 hover:text-slate-850 dark:text-slate-400 dark:hover:text-white transition-colors">Cancel</button>
                 <button
                   onClick={async () => {
