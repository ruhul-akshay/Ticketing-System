...
export default function DepartmentsManagement() {
  const { departments, fetchDepartments, isLoading, error, addDepartment, updateDepartment, deleteDepartment, clearError } = useDepartmentStore();
  ...
  const handleOpenModal = (dept = null) => {
    ...
    if (dept) {
      ...
      setFormData({ name: dept.name, description: dept.description, categories: Array.isArray(dept.categories) ? dept.categories : [] });
    } else {
  ...
  const handleAddCategory = (e) => {
    ...
    if (trimmed) {
      ...
      if (!currentCategories.includes(trimmed)) {
        setFormData({ ...formData, categories: [...currentCategories, trimmed] });
        setCategoryInput('');
      }
    }
  };
  ...
  const handleRemoveCategory = (catToRemove) => {
    const currentCategories = Array.isArray(formData.categories) ? formData.categories : [];
    setFormData({ ...formData, categories: currentCategories.filter(c => c !== catToRemove) });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Automatically add any pending category from the input field
    let updatedCategories = [...formData.categories];
    if (categoryInput.trim() && !updatedCategories.includes(categoryInput.trim())) {
      updatedCategories.push(categoryInput.trim());
    }

    const submissionData = { ...formData, categories: updatedCategories };

    let success;
    if (editingDepartment) {
      success = await updateDepartment(editingDepartment._id, submissionData);
    } else {
      success = await addDepartment(submissionData);
    }
