...
export default function SuperAdminDashboard() {
  ...
  const { clients, fetchClients, isLoading: loadingClients } = useClientStore();
  const { departments, fetchDepartments, isLoading: loadingDepartments } = useDepartmentStore();
  const { clientUsers, fetchClientUsers, isLoading: loadingUsers } = useClientUserStore();
  ...
  const refreshAction = async () => {
    await Promise.all([fetchTickets(), fetchClients(), fetchDepartments(), fetchClientUsers(), fetchLiveStats()]);
  ...
  const isLoading = loadingTickets || loadingClients || loadingDepartments || loadingUsers;

  // Deriving Stats from Stores
  const stats = useMemo(() => {
    const totalUsers = clientUsers.length;
    const activeUsers = clientUsers.filter(u => u.status === 'Active' || u.status === 'active').length;
    const totalTickets = tickets.length;
    const openTickets = tickets.filter(t => t.status === 'Open' || t.status === 'Pending' || t.status === 'On Hold').length;
    const resolvedTickets = tickets.filter(t => t.status === 'Resolved' || t.status === 'Closed').length;
    const totalClients = clients.length;
    const totalDepartments = departments.length;

    return { totalUsers, activeUsers, totalTickets, openTickets, resolvedTickets, totalClients, totalDepartments,
      avgResolutionTime: liveStats.avgResolutionTime,
      avgRating: liveStats.avgRating,
      totalRatings: liveStats.totalRatings
    };
  }, [clientUsers, tickets, clients, departments, liveStats]);
  ...
  return (
    <div className="w-full relative font-sans min-h-screen pt-4 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
        <motion.div onClick={() => navigate('/super-admin/client-users')} initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} transition={{delay:0.05}} className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-3xl shadow-[0_4px_30px_rgba(0,0,0,0.3)] hover:-translate-y-1 transition-all group overflow-hidden relative cursor-pointer">
           ...
           <div className="flex items-center justify-between mb-4 relative z-10">
             ...
             <div className="text-right">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Active Client Users</span>
           ...
           <p className="text-slate-500 text-xs font-bold uppercase tracking-widest relative z-10 mb-4">Total Client Users</p>
        ...
        <motion.div onClick={() => navigate('/super-admin/clients')} initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} transition={{delay:0.15}} className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-3xl shadow-[0_4px_30px_rgba(0,0,0,0.3)] hover:-translate-y-1 transition-all group overflow-hidden relative cursor-pointer">
           ...
           <div className="flex items-center justify-between mb-4 relative z-10">
             ...
             <div className="text-right">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Total Clients</span>
                <span className="text-sm font-black text-emerald-400">{stats.totalClients}</span>
             </div>
           </div>
           <h4 className="text-4xl font-black text-white mb-1 relative z-10">{stats.totalClients}</h4>
           <p className="text-slate-500 text-xs font-bold uppercase tracking-widest relative z-10 mb-4">Total Clients</p>
        ...
        <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} transition={{delay:0.25}} className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 p-6 rounded-3xl shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-1">Global Client User Rating</p>
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-6 sm:gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10 w-full">
         ...
         <div className="bg-[#111620]/80 backdrop-blur-xl border border-white/5 rounded-3xl p-6 shadow-2xl flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
               <h4 className="text-[14px] font-black uppercase text-white tracking-widest flex items-center gap-2"><Users className="text-blue-500" size={16}/> Recent Client Users</h4>
               <span className="text-[10px] font-bold text-slate-500 bg-white/5 px-2 py-1 rounded shadow-inner uppercase tracking-wider">{recentUsers.length} Client Users</span>
            </div>
            <div className="space-y-4 flex-1">
                     ...
                     <div className="flex items-center gap-4 flex-1 min-w-0 pr-4">
                        ...
                        <div className="min-w-0">
                           ...
                           <div className="flex items-center gap-2">
                             ...
                             <span className="text-[10px] font-bold text-blue-400 truncate uppercase tracking-widest">🏢 {u.client?.name || u.clientName || 'N/A'}</span>
                           </div>
                        </div>
                     </div>
             ...
             <div className="mt-6 pt-5 border-t border-white/5 flex flex-col gap-3">
               <div onClick={() => navigate('/super-admin/client-users?status=active')} className="flex items-center justify-between bg-white/[0.02] p-2.5 rounded-lg border border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
                 <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Active Client Users</span>
                 <span className="font-black text-emerald-400 text-sm">{stats.activeUsers}</span>
               </div>
               <div onClick={() => navigate('/super-admin/client-users')} className="flex items-center justify-between bg-white/[0.02] p-2.5 rounded-lg border border-white/5 cursor-pointer hover:bg-white/5 transition-colors">
                 <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Total Client Users</span>
