...
export default function ConsultantKanbanBoard() {
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(t => {
      ...
      return matchesSearch && matchesClient && matchesDepartment && matchesStatus && matchesPriority;
    });
      ...
      <div className="bg-[#111620]/80 backdrop-blur-2xl border border-white/5 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-8 shadow-2xl relative overflow-hidden min-h-[600px] flex flex-col">
        ...
        <div className="bg-[#181f2b]/80 border border-white/5 rounded-2xl p-4 mb-6 relative z-10 flex flex-col gap-4">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <select value={filters.department} onChange={e => {setFilters(p=>({...p, department: e.target.value})); setCurrentPage(1);}} className="bg-[#111620] border border-white/5 rounded-xl px-4 py-3 text-white text-[13px] font-bold focus:outline-none focus:border-blue-500/50 shadow-inner truncate cursor-pointer">
              ...
              {ticketsLoading && currentTickets.length === 0 ? (
                 ...
                 >
                    ...
                    <td className="p-4 align-middle">
                       ...
                       <span className="text-[12px] text-slate-505 font-medium truncate block max-w-[320px]">{t.description}</span>
                    </td>
                    ...
                    <td className="p-4 align-middle text-right">
                       <div className="flex items-center justify-end gap-2">
                          <button 
                            ...
                            className="text-[10px] font-bold text-slate-400 hover:text-white bg-white/5 px-3 py-1.5 rounded-lg border border-transparent shadow-sm hover:border-white/10 uppercase tracking-widest transition-all"
                          >
                            View
                          </button>
                          {t.status !== 'Resolved' && (
