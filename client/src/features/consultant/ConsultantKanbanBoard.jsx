...
import { Plus, RefreshCw, Layers, CheckCircle, Clock, XCircle, Ticket } from 'lucide-react';
...
export default function ConsultantKanbanBoard() {
  ...
  const stats = useMemo(() => {
    ...
    const open = tickets.filter((t) =>
      ['open', 'pending', 'assigned'].includes(t.status?.toLowerCase())
    ).length;
  ...
  return (
    <div className="w-full space-y-6 sm:space-y-8 font-sans pb-12">
      ...
      <PageHeader
        ...
        subtitle="Manage, triage, and resolve incoming technical support tickets."
        icon={Layers}
