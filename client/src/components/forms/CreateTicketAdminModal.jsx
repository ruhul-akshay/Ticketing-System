...
export default function CreateTicketAdminModal({ isOpen, onClose }) {
  ...
  // Auto-select company if only one is available (e.g. for Admins)
  useEffect(() => {
    if (isOpen && companies.length === 1 && !formData.companyId) {
      const compId = companies[0]._id || companies[0].id;
      setFormData(prev => ({ ...prev, companyId: compId }));
    }
  }, [isOpen, companies, formData.companyId]);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const res = await api.get(`/users?companyId=${formData.companyId}`);
        const users = Array.isArray(res.data) ? res.data : (res.data.users || res.data.data || []);
        
        // Allow any user from this company (both 'user' and 'admin' roles)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.companyId || !formData.userId || !formData.departmentId || !formData.title || !formData.description) {
      alert("Validation Error: Please select a company, ensure a user is available, assign a department, and provide a title and description.");
      return;
    }
        ...
        >
          <div className="px-8 py-6 border-b border-white/5 bg-[#181f2b]/50 backdrop-blur-xl flex justify-between items-center sticky top-0 z-20">
            <div>
              ...
              <p className="text-[13px] font-semibold text-slate-400 mt-1 uppercase tracking-wider">Log an issue manually via Admin delegation</p>
            </div>
            <button 
              type="button"
              onClick={onClose} 
             ...
             <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="space-y-2">
                   <label className="text-[13px] font-bold text-slate-300 uppercase tracking-widest">Company Framework *</label>
                   <select 
                 ...
                 <div className="space-y-2">
                   ...
                   >
                     <option value="">{companyUsers.length === 0 ? (formData.companyId ? 'Loading Users...' : 'Select Company First') : 'Select User Array'}</option>
                     {companyUsers.map(u => <option key={u._id || u.id} value={u._id || u.id}>{u.name || u.email}</option>)}
                   </select>
                 </div>
               </div>
