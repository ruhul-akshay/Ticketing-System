...
export default function ManageUserModal({ isOpen, mode, user, onClose, companies }) {
  const { addUser, updateUser } = useUserStore();
  const [isLoading, setIsLoading] = useState(false);
  
  const [formData, setFormData] = useState({
    name: '', email: '', password: '', confirmPassword: '', employeeCode: '',
    companyId: '', phoneNumber: '', position: ''
  });
  ...
  useEffect(() => {
    if (mode === 'edit' && user) {
      ...
      setFormData({ name: '', email: '', password: '', confirmPassword: '', employeeCode: '', companyId: '', phoneNumber: '', position: '' });
    }
  }, [mode, user]);
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      employeeCode: formData.employeeCode, companyId: formData.companyId,
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} className="bg-[#111620] border border-white/10 w-full max-w-2xl rounded-[2rem] shadow-[0_0_80px_rgba(0,0,0,0.6)] relative z-10 flex flex-col overflow-hidden max-h-[90vh]">
        <div className="px-8 py-6 border-b border-white/5 bg-[#181f2b]/50 backdrop-blur-xl flex justify-between items-center sticky top-0 z-20">
          <div>
            ...
            <p className="text-[13px] font-semibold text-slate-400 mt-1 uppercase tracking-wider">{mode === 'create' ? 'Provision a new user into the platform' : `Editing record ${user?.id || user?._id || ''}`}</p>
          </div>
        ...
        <div className="p-8 overflow-y-auto relative custom-scrollbar flex-1">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input label="Full Core Name *" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} required />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[13px] font-bold text-slate-300 uppercase tracking-widest mb-2 block">Assigned Company Scope</label>
                <select value={formData.companyId} onChange={e => setFormData(p => ({ ...p, companyId: e.target.value }))} className="w-full bg-[#1d2633] border border-white/5 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500/50 shadow-inner appearance-none text-sm font-medium">
